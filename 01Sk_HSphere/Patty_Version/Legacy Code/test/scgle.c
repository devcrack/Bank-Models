...
int allocate_some_Memory(scgle *data, FILE * input_Ops){
  ...
  if (!input) 
    return 0;
  printf("memoria = %d\n",input_Ops);
  fscanf(input_Ops, "%s", input); /* Line 1 */
  data->kMax = atoi(input);
  fscanf(input_Ops, "%s", input); /* Line 2 */
  data->deltaK = atof(input);
  fscanf(input_Ops, "%s", input); /* Line 3 */
  printf("Phi = %s\n",input);
  data->phi = atof(input);
  fscanf(input_Ops, "%s", input);
  data->deltaT = atof(input);
  fscanf(input_Ops, "%s", input);
  
  data->kas = (int)(data->kMax/data->deltaK);
