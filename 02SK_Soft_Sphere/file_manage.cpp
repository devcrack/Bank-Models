...
void
File_Manage::save_Sk_SSPhere_V2(int nKas,double phi,double deltaK,double _dss ,double *sk) {
  double k,i;
  int index;
  ...
  for(
    ...
    // this->outfile << std::fixed << std::setprecision(18) << sk[index] << "\n"; // Original Line Code
    this->outfile << std::scientific << sk[index] << "\n"; // Original Line Code
  }
  outfile.clear();
  outfile.close();
}
...
void
File_Manage::truncate_cfg_file() {
  outfile.open("./config/options_SSphere.txt", std::ofstream::trunc);
...
File_Manage::load_config(const std::string vol_fract, const std::string tmp) {  
  this->outfile.open("./config/options_SSphere.txt");
